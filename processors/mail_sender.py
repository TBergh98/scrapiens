...
Also tracks which grants have been successfully sent (with email_delivered=True) to prevent
re-sending the same grant to the same recipient across different pipeline runs.
"""
...
class MailSender:
    ...
    def __init__(
        ...
        dry_run: bool = False,
        sent_grants_manager: Optional[SentGrantsManager] = None,
    ) -> None:
        """
        ...
        Args:
            ...
            sent_grants_manager: Manager for tracking sent grants. If None, creates default.
        """
    ...
    def _send_single_digest(self, digest: Dict[str, Any]) -> None:
        """Send a single digest to its recipient and track sent grants."""
        recipient = digest["email"]
        html_body = digest.get("html_body", "")
        text_body = digest.get("text_body", "")
        grants = digest.get("grants", [])
        ...
        try:
            ...
            # Track sent grants in history
            if not self.dry_run and grants:
                for grant in grants:
                    grant_url = grant.get("url")
                    if grant_url:
                        self.sent_grants_manager.mark_sent(
                            grant_url=grant_url,
                            recipient_email=recipient,
                            email_delivered=True  # Only mark as delivered if we got here
                        )
                logger.debug(f"Tracked {len(grants)} grants as sent to {recipient}")

...
from utils.file_utils import aggregate_link_files, save_json, load_json
...
def deduplicate_from_directory(
    input_dir: Path,
    output_file: Path,
    file_pattern: str = "*_links.txt"
) -> Dict[str, Any]:
    """
    Load all link files from a directory, deduplicate, and save results.
    
    Args:
        input_dir: Directory containing individual link files
    ...
    # Load all link files
    all_links = aggregate_link_files(input_dir, file_pattern)
    ...
    # Deduplicate
    results = deduplicate_links(all_links)
...
def merge_deduplication_results(result_files: List[Path]) -> Dict[str, Any]:
    ...
    merged = deduplicate_links(all_sites)
